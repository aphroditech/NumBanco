...
function pickWeightedSlot(weights) {
...
export function rollPlinkoSlot(rows) {
    const n = Math.max(8, Math.min(16, Math.round(rows)));
    if (n === 8) {
        return pickWeightedSlot(PLINKO_ROW8_SLOT_WEIGHTS);
    }
    if (n === 9) {
        return pickWeightedSlot(PLINKO_ROW9_SLOT_WEIGHTS);
    }
    if (n === 10) {
        return pickWeightedSlot(PLINKO_ROW10_SLOT_WEIGHTS);
    }
    if (n === 11) {
        return pickWeightedSlot(PLINKO_ROW11_SLOT_WEIGHTS);
    }
    if (n === 12) {
        return pickWeightedSlot(PLINKO_ROW12_SLOT_WEIGHTS);
    }
    if (n === 13) {
        return pickWeightedSlot(PLINKO_ROW13_SLOT_WEIGHTS);
    }
    if (n === 14) {
        return pickWeightedSlot(PLINKO_ROW14_SLOT_WEIGHTS);
    }
    if (n === 15) {
        return pickWeightedSlot(PLINKO_ROW15_SLOT_WEIGHTS);
    }
    if (n === 16) {
        return pickWeightedSlot(PLINKO_ROW16_SLOT_WEIGHTS);
    }
    let rights = 0;
    for (let i = 0; i < n; i += 1) {
