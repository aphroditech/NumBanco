...
function PlinkoBetHistory({ results }) {
    const list = Array.isArray(results) ? results : [];

    function reverseArray(arr) {
        return [...arr].reverse();
    }
    const reversedResults = reverseArray(list);

    const [currentPage, setCurrentPage] = useState(1);
    const [itemsPerPage, setItemsPerPage] = useState(5);

    const totalPages = useMemo(() => Math.ceil(reversedResults.length / itemsPerPage) || 1, [reversedResults.length, itemsPerPage]);
                            ...
                            <Table variant="simple" color="#fff" height="100%" width="100%" sx={{ tableLayout: 'fixed' }}>
                                <Thead top="0" zIndex="5">
                                    <Tr>
                                        ...
                                        <Th color="white" textAlign="left" className="real_th_font" width="16%">
                                            Mult
                                        </Th>
                                        <Th color="white" textAlign="left" className="real_th_font" width="12%">
                                            Rows
                                        </Th>
                                        <Th color="white" textAlign="left" className="real_th_font" width="18%">
                                            Amount
                                        </Th>
                                        <Th color="white" textAlign="left" className="real_th_font" width="18%">
                                            Profit
                                        </Th>
                                        <Th color="white" textAlign="left" className="real_th_font" width="28%">
                                            Time
                                        </Th>
                                    </Tr>
                                </Thead>
                                <Tbody>
                                    {paginatedResults.map((result, index, arr) => {
                                        ...
                                        const created = result.createAt ? new Date(result.createAt) : null;
                                        return (
                                            <Tr key={result.roundId ?? result.id ?? globalIndex}>
                                                <Td
                                                    textAlign="left"
                                                ...
                                                <Td
                                                    ...
                                                    fontWeight="normal"
                                                >
                                                    x{truncateToTwo(result.multiplier)}
                                                </Td>
                                                <Td
                                                    textAlign="left"
                                                    border={lastItem ? 'none' : null}
                                                ...
                                                <Td
                                                    ...
                                                    fontWeight="normal"
                                                >
                                                    {formatUsdDisplay(result.profit)}
                                                </Td>
                                                <Td
                                                    textAlign="left"
                                                    border={lastItem ? 'none' : null}
...
export default PlinkoBetHistory;
