...
import { getAToZHistory } from 'action/AtoZActions';
...
function UserBetHistory() {
    ...
    const history = useSelector((state) => state.histories.aToZHistory) || [];
    useEffect(() => {
        setResults(history);
    }, [history]);
    // results.reverse();
    ...
    useEffect(() => {
        // `getAToZHistory` is async and calls `dispatch` internally — do not `dispatch(promise)`.
        void getAToZHistory(History, dispatch);
    }, [dispatch, History]);
