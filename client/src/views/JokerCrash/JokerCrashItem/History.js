...
import JokerCrashHistoryRow from 'components/Tables/JokerCrashHistoryRow';
...
function History() {
    ...
    const sortedHistory = user.jokerCrashHistory?.filter((item) => item.active)?.length > 0
        ? user.jokerCrashHistory.filter((item) => item.active).sort((a, b) => {
