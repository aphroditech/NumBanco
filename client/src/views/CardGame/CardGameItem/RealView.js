...
import CardGameRealViewRow from 'components/Tables/CardGameRealViewRow';
import { getCardGameView } from 'action/CardGameActions';
import { useAblyCardGameUpdates } from 'hooks/useAblyCardGameUpdates';
import { useHistory } from 'react-router-dom';

function RealView() {
    const { cardGameView, setCardGameView } = useAblyCardGameUpdates();
    ...
    useEffect(() => {
        getCardGameView(history).then((res) => {
            const raw = res?.data?.data ?? res?.data;
            setCardGameView(normalizeViewPayload(raw));
        });
    }, []);

    // Detect new rows and apply animation
    useEffect(() => {
        if (!cardGameView || cardGameView.length === 0) {
        ...
        const currentIds = new Set(cardGameView.map((row, i) => getRowId(row, i)));
    ...
    const maxRows = 12;
    const baseRows = Array.isArray(cardGameView) ? cardGameView : [];
