...
import CryptoCrashHistoryRow from 'components/Tables/CryptoCrashHistoryRow';
...
function History() {
    ...
    const sortedHistory = user.cryptoCrashHistory?.filter((item) => item.active)?.length > 0
        ? user.cryptoCrashHistory.filter((item) => item.active).sort((a, b) => {
                        ...
                        >
                            ...
                            <Tbody>
                                {paginatedHistory.map((row, index, arr) => {
                                    ...
                                    return (
                                        <CryptoCrashHistoryRow
                                            key={globalIndex}
                                            No={globalIndex + 1}
                                            multi={row.multi.toFixed(2)}
                                            bet={row.bet}
                                            win={row.win.toFixed(2)}
                                            time={row.createAt}
