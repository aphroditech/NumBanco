...
import CryptoCrashRealViewRow from 'components/Tables/CryptoCrashRealViewRow';
import StarRoundedIcon from '@mui/icons-material/StarRounded';
import { getCryptoCrashView } from 'action/CryptoCrashActions';
import { useAblyCryptoCrashUpdates } from 'hooks/useAblyCryptoCrashUpdates';
import { useHistory } from 'react-router-dom';

function RealView() {
    const { cryptoCrashView, setCryptoCrashView } = useAblyCryptoCrashUpdates();
    ...
    useEffect(() => {
        getCryptoCrashView(history).then((res) => {
            setCryptoCrashView(res.data);
        });
    }, []);

    // Detect new rows and apply animation
    useEffect(() => {
        if (!cryptoCrashView || cryptoCrashView.length === 0) {
        ...
        const currentIds = new Set(cryptoCrashView.map(getRowId));
    ...
    const maxRows = 12;
    const baseRows = Array.isArray(cryptoCrashView) ? cryptoCrashView : [];
