...
import FishingHistoryRow from 'components/Tables/FishingHistoryRow';
...
function History() {
    ...
    const sortedHistory = user.fishingHistory?.filter((item) => item.active)?.length > 0
        ? user.fishingHistory.filter((item) => item.active).sort((a, b) => {
