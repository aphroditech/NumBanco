...
import { getClimbView } from "action/ClimbActions";
import { useAblyClimbUpdates } from "hooks/useAblyClimbUpdates";
import { useHistory } from "react-router-dom";

/** Same layout and row UI as Twist RealView; data from `ClimbView` + Ably `climbGame`. */
function ClimbRealView() {
    const { climbView, setClimbView } = useAblyClimbUpdates();
    ...
    useEffect(() => {
        getClimbView(history).then((res) => {
            const raw = res?.data?.data ?? res?.data;
            setClimbView(normalizeViewPayload(raw));
        });
    }, [history]);

    useEffect(() => {
        if (!climbView || climbView.length === 0) {
        ...
        const currentIds = new Set(climbView.map((row, i) => getRowId(row, i)));
    ...
    const maxRows = 13;
    const baseRows = Array.isArray(climbView) ? climbView : [];
