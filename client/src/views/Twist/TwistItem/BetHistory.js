...
/** Same layout as Alpha Tree BetHistory; data from `user.twistHistory` (cash-out rows). */
function BetHistory() {
    const user = useSelector((state) => state.user.userInfo) || {};

    /** Latest first */
    const sortedHistory = useMemo(() => {
        const raw = user.twistHistory;
                        ...
                        >
                            ...
                            <Tbody>
                                {paginatedHistory.map((row, index, arr) => {
                                    ...
                                    const rowKey = `${row.createAt}-${row.betAmount}-${row.profit}-${globalIndex}`;
                                    return (
                                        <AlphaTreeHistoryRow
                                            key={rowKey}
                                            No={globalIndex + 1}
