...
import { useAblyTarotUpdates } from "hooks/useAblyTarotUpdates";
import { useHistory } from "react-router-dom";

/** Same layout as `DiamondRealView`; data from `tarotviews` + Ably `tarotGame`. */
function TarotRealView({ suppressFeedUntil = 0 }) {
    const history = useHistory();
    const { tarotView, setTarotView } = useAblyTarotUpdates({ suppressFeedUntil, history });
    const [newRowIds, setNewRowIds] = useState(new Set());
    const prevRowIdsRef = useRef(new Set());
    const hasInitializedRef = useRef(false);

    const getRowId = (row, index) => {
        if (!row) return `tv-empty-${index}`;
        if (row._id != null) return String(row._id);
        if (row.id != null) return String(row.id);
        const t = row.time ?? row.createdAt ?? "";
        const uid = row.userId ?? "u";
        return `tv-${uid}-${t}-${row.result ?? ""}-${row.win ?? ""}-${index}`;
    };
    ...
    useEffect(() => {
        getTarotLiveView(history).then((res) => {
    ...
    useEffect(() => {
        if (!tarotView || tarotView.length === 0) {
        ...
        const currentIds = new Set(tarotView.map((row, i) => getRowId(row, i)));
    ...
    const maxRows = 14;
    const baseRows = Array.isArray(tarotView) ? tarotView : [];
