...
export const initializeFishingLimits = async () => {
  try {
    const count = await FishingLimit.countDocuments();

    if (count === 0) {
      await FishingLimit.insertMany([
    ...
    console.error('❌ Error initializing Fishing Limits:', error);
  }
};
