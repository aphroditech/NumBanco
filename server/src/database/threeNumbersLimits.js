...
export const initializeThreeNumbersLimits = async () => {
  try {
    const count = await ThreeNumbersLimit.countDocuments();

    if (count === 0) {
      await ThreeNumbersLimit.insertMany([
    ...
    console.error('❌ Error initializing ThreeNumbers Limits:', error);
  }
};
