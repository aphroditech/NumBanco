...
export const initializeCryptoCrashLimits = async () => {
  try {
    const count = await CryptoCrashLimit.countDocuments();

    if (count === 0) {
      await CryptoCrashLimit.insertMany([
    ...
    console.error('❌ Error initializing CryptoCrash Limits:', error);
  }
};
